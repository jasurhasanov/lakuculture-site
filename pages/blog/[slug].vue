    ...
    <main class="shell article-body">
      <aside class="article-brief">
        <dl>
          ...
          <div>
            <dt>Conversion action</dt>
            <dd>{{ post.conversionAction }}</dd>
          </div>
        </dl>
      </aside>
      ...
      <section>
        <h2>Why this search has real booking intent</h2>
        <p>
          The query "{{ post.intentQuery }}" usually comes from someone who is already comparing real options, not only collecting ideas. For {{ post.audience.toLowerCase() }}, the next useful answer is practical: where the experience is, who it suits, how long it might take, what needs to be booked, and why LAKU is worth choosing over another beach, cafe, or generic tour.
        </p>
        <p>
          This is why the article focuses on {{ post.primaryKeyword }} while also answering related searches such as {{ post.secondaryKeywords.join(', ') }}. The goal is not to repeat keywords mechanically. The goal is to remove hesitation and make the right next action obvious for the reader.
        </p>
      </section>
      ...
      <section>
        ...
        <p>
          The conversion step is simple: {{ post.conversionAction }} Send the inquiry while your dates are still flexible, then let the team recommend the workshop, visit, or Culture Keeper session that fits your intention.
        </p>
      </section>
