...
const guestActivities = [
  { id: "nyensek-germany", copy: "Anna from Germany booked a Nyensek weaving workshop." },
  { id: "bamboo-australia", copy: "James from Australia asked about a handmade bamboo piece." },
  { id: "cooking-singapore", copy: "Sofia from Singapore requested a local cooking class." },
  { id: "private-uk", copy: "Michael from the UK planned a private Culture Keeper visit." },
  { id: "hat-usa", copy: "Maya from the USA asked about a custom woven hat." },
  { id: "group-france", copy: "Camille from France requested a private group workshop." }
] as const
...
const guestActivitySessionKey = "laku-guest-activity-shown"
...
const pickGuestActivity = () => {
  const availableActivities = guestActivities.filter((activity) => !wasStoredRecently(guestActivityDismissPrefix + activity.id, guestActivityDismissDays))
...
const showGuestActivity = () => {
  ...
  if (window.sessionStorage.getItem(guestActivitySessionKey)) return
  ...
  window.sessionStorage.setItem(guestActivitySessionKey, "1")
  window.localStorage.setItem(guestActivityDismissPrefix + activity.id, String(Date.now()))
}
